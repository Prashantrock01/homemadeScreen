...
import 'package:biz_infra/Screens/poster/poster_creation.dart';
...
// Determine the notice type
    final advNoticeType = helpDeskTicketStatus.isEmpty
        ? "All"
    ...
    final filter = advNoticeType == "All"
        ? ""
        : helpDeskTicketStatus.isEmpty
            ? "[[\"advnotice_type\",\"e\",\"$advNoticeType\"]]"
            : "[[\"advnotice_type\",\"e\",\"$advNoticeType\"],[\"ticketstatus\",\"e\",\"${helpDeskTicketStatus.value}\"]]";

    try {
      final data = await dioServiceClient.getAdvNoticeList(
        page: page,
        filter: filter,
      );
      // pass page here
      if (data != null && data.data != null) {
        setState(() {
          ...
          hasMoreRecords = data
              .data!.moreRecords!; // Check if there are more records to load
        });
      }
    } catch (e) {
  ...
  void filterRecords(String query) {
    setState(() {
      records = allRecords
          .where((record) =>
              (record.advnoticeSub != null &&
                  record.advnoticeSub!
                      .toLowerCase()
                      .contains(query.toLowerCase())) ||
              (record.advDescription != null &&
        ...
        itemBuilder: (context, index) {
          return Container(
            ...
            child: Card(
              ...
              child: InkWell(
                onTap: () {
                  if (filteredRecords[index].posterApprovalStatus?.trim() !=
                      'Pending') {
                    Get.to(
                      () => PosterDetails(
                        entryId: filteredRecords[index].id!,
                        noticeId: filteredRecords[index].advnoticeid!,
                      ),
                      transition: Transition.rightToLeft,
                      popGesture: true,
                    );
                    ...
                    if (kDebugMode) {
                      print('Cannot view details for pending status.');
                    }
                  }
                },
                child: Container(
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          Flexible(
                            child: Text(
                              filteredRecords[index].advnoticeSub ?? '',
                              softWrap: true,
                              style: const TextStyle(
                      ...
                      Text(
                        filteredRecords[index].advDescription ?? '',
                        style: const TextStyle(
                          fontSize: 11,
                        ),
                      ),
                      ListTile(
                        ...
                        trailing: (filteredRecords[index]
                            ...
                            : Badge(
                                ...
                                label: Text(
                                  filteredRecords[index].posterApprovalStatus ??
                                      '',
                                ),
                              ),
                      ),
