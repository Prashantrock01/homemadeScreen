...
import '../../Profile/edit_email.dart';

class InfraEmpProfile extends StatefulWidget {
  const InfraEmpProfile({super.key});

  @override
  InfraEmpProfileState createState() => InfraEmpProfileState();
}

class InfraEmpProfileState extends State<InfraEmpProfile> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(
        ...
        child: FutureBuilder(
          future: dioServiceClient.infraEmpProfileApi(),
          builder: (context, snapshot) {
            if (snapshot.hasError) {
              ...
              final InfraProfileInfo? profile = snapshot.data!.data!.profileInfo;
  ...
  Widget buildProfileDetails(InfraProfileInfo? profile) {
    return Column(
      ...
      children: [
        ...
        keyValue('Name', profile.infraempName.toString()),
        keyValue('Phone', profile.infraempMobile.toString()),
        keyValue(
          'Email',
          profile.infraempEmail.toString(),
          trailingIcon: InkWell(
            onTap: () => Get.to(() => EditEmailScreen(email: profile.infraempEmail.toString())),
            child: const Icon(Icons.edit, size: 20),
          ),
        ),
        keyValue('Address', '${profile.infraempAddress}, ${profile.infraempCity}, ${profile.infraempDistrict}, ${profile.infraempState} - ${profile.infraempPincode}'),
        keyValue('Work Hours', '${formatTime(profile.infraempStarttime!)} - ${formatTime(profile.infraempEndtime!)}'),
        keyValue('Week Off', profile.infraempWeekOff!),
      ],
    );
  }

  String formatTime(String time24) {
    DateTime parsedTime = DateFormat("hh:mm:ss").parse(time24); // Dummy date
